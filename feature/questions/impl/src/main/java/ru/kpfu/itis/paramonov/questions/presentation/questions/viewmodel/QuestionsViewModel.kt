...
import ru.kpfu.itis.paramonov.questions.presentation.questions.fragments.ResultFragment
...
class QuestionsViewModel(
            ...
            try {
                ...
                for (pos in questionData.indices) {
                    _questionList.add(pos,
                        MutableStateFlow(
                            questionDataUiModelMapper.map(questionData[pos])
                        )
                    )
                }
    ...
    private fun saveQuestions() {
        viewModelScope.launch {
            ...
            saveQuestionsUseCase.invoke(questions.map { question -> questionDataApiModelMapper.map(question) })
        }
    }
}