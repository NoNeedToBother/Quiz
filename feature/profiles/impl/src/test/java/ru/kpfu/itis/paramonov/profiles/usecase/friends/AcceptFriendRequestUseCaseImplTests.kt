...
class AcceptFriendRequestUseCaseImplTests {

    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun acceptFriendRequestUseCaseImpl_success() = runTest {
        val dispatcher = UnconfinedTestDispatcher()
        val id = "1"
        val friendRepository = mock<FriendRepository> {
            onBlocking { acceptFriendRequest(id) } doReturn Unit
        }

        val impl = AcceptFriendRequestUseCaseImpl(dispatcher, friendRepository)
        impl.invoke(id)

        verify(friendRepository).acceptFriendRequest(id)
    }
}