...
class GetUserLastResultsUseCaseImplTests {
    ...
    fun getCurrentUserLastResultsUseCaseImpl_success_someResults() = runTest {
        ...
        val resultRepository = mock<ResultRepository> {
            onBlocking { getLastResults(anyInt(), anyString()) } doReturn mockedResults
        }

        val impl = GetUserLastResultsUseCaseImpl(resultRepository, dispatcher)
        val results = impl.invoke(10, "1")

        verify(resultRepository).getLastResults(10, "1")
    ...
    fun getCurrentUserLastResultsUseCaseImpl_success_emptyList() = runTest {
        ...
        val resultRepository = mock<ResultRepository> {
            onBlocking { getLastResults(anyInt(), anyString()) } doReturn emptyList()
        }
