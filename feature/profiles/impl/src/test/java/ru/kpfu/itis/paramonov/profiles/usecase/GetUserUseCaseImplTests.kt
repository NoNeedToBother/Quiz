...
class GetUserUseCaseImplTests {
    ...
    fun getCurrentUserUseCaseImplTests_noUser() = runTest {
        ...
        val userRepository = mock<UserRepository> {
            onBlocking { getUser(anyString()) } doReturn null
        }

        val impl = GetUserUseCaseImpl(dispatcher, userRepository, resourceManager)
    ...
    fun getCurrentUserUseCaseImplTests_error() = runTest {
        ...
        val userRepository = mock<UserRepository> {
            onBlocking { getUser(anyString()) } doThrow RuntimeException()
        }
    ...
    fun getCurrentUserUseCaseImplTests_success() = runTest {
        ...
        val userRepository = mock<UserRepository> {
            onBlocking { getUser(anyString()) } doReturn mockedUser
        }

        val impl = GetUserUseCaseImpl(dispatcher, userRepository, resourceManager)
        val user = impl.invoke("1")
